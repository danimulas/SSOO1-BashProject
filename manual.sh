...
turno() {
    ...
    if [ "$jugadorTurno" -eq "0" ]; then
        echo "Ningún jugador puede jugar la carta $numeroEliminar."
    else
...
bucle_jugabilidad() {
    ...
    if [ "$jugadorTurno" -eq "1" ]; then
        ...
        echo "MAX: $max_mesaOros MIN: $min_mesaOros"
        jugar_manual
    fi
    if [ "$jugadorTurno" -eq "2" ]; then
...
jugar_manual(){
    ...
    #); then
        ...
        echo "Elige el numero de la carta que quieres hechar, si no quieres hechar ninguna carta introduce -1" #ESTO DE NO INTRODUCIR NO VALE
        read -p "Elige una carta por su posición: " posicion
        if [ "$posicion" -eq -1 ]; then
            pasar_turno
            echo "Se le pasa el turno al jugador $jugadorTurno"
            return
        fi
        if [[ $posicion -ge 0 && $posicion -lt $cartasJugadorActual ]]; then
            carta=${cartasJugadorActual[$posicion]}
            echo "Carta elegida: $carta"

            # Utilizar declare -p para imprimir la declaración de la variable
            declare -p carta
            ...
            if [ "$carta" -eq "$((min_mesaOros - 1))" ] || ( [ "$carta" -le 10 ] && [ "$carta" -eq "$((max_mesaOros + 1))" ] ) || [ "$carta" -ge 11 ] && [ "$carta" -eq "$((min_mesaCopas - 1))" ] || [ "$carta" -le 20 ] && [ "$carta" -eq "$((max_mesaCopas + 1))" ] || [ "$carta" -ge 21 ] && [ "$carta" -eq "$((min_mesaEspadas - 1))" ] || [ "$carta" -le 30 ] && [ "$carta" -eq "$((max_mesaEspadas + 1))" ] || [ "$carta" -ge 31 ] && [ "$carta" -eq "$((min_mesaBastos - 1))" ] || [ "$carta" -eq "$((max_mesaBastos + 1))" ]; then
                    carta_elegida="${cartasJugadorActual[$posicion]}"
                    echo "Has elegido la carta: $carta_elegida"
                    numeroEliminar="$carta"
                    carta_valida=true 
            fi
        else
            echo "Posición no válida. Por favor, elige una posición válida."
        fi
    #else
     #    echo "El jugador $jugadorTurno no puede jugar ninguna carta."
      #  pasar_turno
